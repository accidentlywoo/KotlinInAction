...
코틀린에서는 프로그래머 대신 컴파일러가 캐스팅을 해준다.:heart_eyes:
...
'블록의 마지막 식이 블록의 결과' 라는 규칙은 블록이 값을 만들어내야 하는 경우 항상 성립한다.:pushpin:
...
for는 자바의 for-each 루프에 해당하는 형태만 존재한다.:pushpin:
...
***..*** 연산자로 시작 값과 끝 값을 연결해서 범위를 만든다 :heart_eyes:
...
나중에 downTo, step, until에 대해 더 자세히 다룬다.:pushpin:
...
그런 구조 분해 구문을 사용하면 원소의 현재 인덱스를 유지하면서 컬렉션을 이터레이션할 수 있다.:heart_eyes:

- [맵, 리스트 콜렉션 예제보러 가기](https://github.com/accidentlywoo/KotlinInAction/blob/main/practice/src/test/kotlin/chapter2/IterationTest.kt)

- in 으로 컬렉션이나 범위의 원소 검사 :heart_eyes:

in 연산자를 사용해 어떤 값이 범위에 속하는지 검사할 수 있다.
  
반대로 !in을 사용하면 어떤 값이 범위에 속하지 않는지 검사할 수 있다.:heart_eyes:
